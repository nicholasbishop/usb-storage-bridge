...
// In this initial debugging stage I stall on an un-successful system call, else
// I display progress
{
...
void BackgroundPrint(uint32_t TimeToWait) {
  ...
  if (TimeToWait > 10) {
    // Check more often so that the events are better displayed in the order
    // that they appeared
    Limit = TimeToWait / 10;
    TimeToWait = 10;
  } else
  ...
  for (Count = 0; Count < Limit; Count++) {
    ActualEvents = 0;
    Status = CyU3PEventGet(&DisplayEvent, 0xFFFF, TX_OR_CLEAR, &ActualEvents,
                           TimeToWait);
