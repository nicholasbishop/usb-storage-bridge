...
/*
   The following definitions specify the start address and length of the Driver
...
/*
   The last 32 KB of RAM is reserved for 2-stage boot operation. This value can
   be changed to 0x40040000 if 2-stage boot is not used by the application.
 */
...
/*
   The last 32 KB of RAM is reserved for 2-stage boot operation. This value can
   be changed to 0x40080000 if 2-stage boot is not used by the application.
 */
...
/*
   ...
   the FX3 device. The reference implementation of the buffer allocator makes
...
/* This function initializes the custom heap for OS specific dynamic memory
 * allocation. The function should not be explicitly invoked. This function is
 * called from the API library. Modify this function depending upon the heap
 * requirement of application code. The minimum required value is specified by
 * the predefined macro. Any value less than specified can cause the drivers to
 * stop functioning. The function creates a global byte pool.
 */
void CyU3PMemInit(void) {
  if (!glMemPoolInit) {
    glMemPoolInit = CyTrue;
    CyU3PBytePoolCreate(&glMemBytePool, CY_U3P_MEM_HEAP_BASE,
                        CY_U3P_MEM_HEAP_SIZE);
  }
}
 ...
 * If other buffer sizes are required by the application code, this function
 * must be modified to create other block pools.
 */
...
void* CyU3PDmaBufferAlloc(uint16_t size) {
  ...
  /* Find the number of 32 byte chunks required. The minimum size that can be
     handled is 64 bytes. */
    ...
    if ((glBufferManager.usedStatus[wordnum] & (1 << bitnum)) == 0) {
      ...
      if (count == (size + 1)) {
        /* The last bit corresponding to the allocated memory is left as zero.
           This allows us to identify the end of the allocated block while
           freeing the memory. We need to search for one additional zero while
           allocating to account for this hack. */
...
int CyU3PDmaBufferFree(void* buffer) {
  ...
  /* If the buffer address is within the range specified, count the number of
     consecutive ones and clear them. */
  start = (uint32_t)buffer;
  if ((start > glBufferManager.startAddr) &&
      (start < (glBufferManager.startAddr + glBufferManager.regionSize))) {
    ...
    /* Start the next buffer search at the top of the heap. This can help reduce
       fragmentation in cases where
